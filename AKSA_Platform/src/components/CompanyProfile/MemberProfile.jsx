      ...
      <section className={`${cardBg} rounded-lg shadow p-8 mb-8 ${border}`}>
        <h2 className="text-lg font-semibold mb-6 flex items-center gap-2 text-primary">
          <span className="inline-block w-5 h-5 bg-purple-100 rounded-full flex items-center justify-center mr-2">
            <svg width="16" height="16" fill="none" viewBox="0 0 16 16"><path d="M2.5 13.5v-2A2.5 2.5 0 0 1 5 9h6a2.5 2.5 0 0 1 2.5 2.5v2M8 7A2.5 2.5 0 1 0 8 2a2.5 2.5 0 0 0 0 5Z" stroke="#7C3AED" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </span>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
          <div className="flex flex-col items-center">
            <div className="w-32 h-32 rounded-full bg-purple-100 flex items-center justify-center mb-2 relative overflow-hidden border-4 border-purple-200">
              <img src={profileData.logo} alt="Company Logo" className="object-cover w-full h-full" />
            </div>
            <input
            ...
            <div className="flex gap-2 mt-2">
              <button
                className="px-4 py-1 bg-primary text-white rounded hover:bg-[#800080] text-sm flex items-center gap-1 shadow"
                onClick={() => document.getElementById('logo-upload-input').click()}
                type="button"
              >
                <svg width="16" height="16" fill="none" viewBox="0 0 16 16"><path d="M8 10.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z" stroke="#fff" strokeWidth="1.5" /><path d="M13.5 14v-1.25A2.75 2.75 0 0 0 10.75 10h-5.5A2.75 2.75 0 0 0 2.5 12.75V14" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" /></svg>
                Upload
              </button>
            </div>
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-gray-900">{domainCount} <span className="text-primary underline cursor-pointer ml-2 text-sm" onClick={() => setShowDomainsInline(true)}>View my domains</span></div>
            </div>
            <div>
      ...
      <section className={`${cardBg} rounded-lg shadow p-8 mb-8 ${border}`}>
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
          <div className="flex-1 flex items-center border-2 border-dashed border-dashed rounded-lg p-4 bg-purple-50 justify-center">
            ...
            <button
              className="ml-2 px-4 py-1 bg-primary text-white rounded hover:bg-[#800080] text-sm flex items-center gap-1 shadow"
              onClick={() => document.getElementById('attachment-upload-input').click()}
              type="button"
            >
              <svg width="16" height="16" fill="none" viewBox="0 0 16 16"><path d="M8 3v7m0 0l-2.5-2.5M8 10l2.5-2.5M3 13h10" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" /></svg>
              Browse
            </button>
          </div>
          <div className="w-full md:w-72">
            <input type="text" placeholder="Search an attachment" className="w-full border border-purple-200 rounded-lg px-3 py-2 text-sm focus:outline-none bg-purple-50 text-purple-900" />
          </div>
        </div>
        <div className="overflow-x-auto rounded-lg border border-purple-100">
          <table className="min-w-full text-sm text-left">
            ...
            <tbody>
              {/* Placeholder row */}
              <tr>
                <td colSpan={6} className="text-center py-8 text-secondary font-semibold">Attachments will appear here</td>
              </tr>
            </tbody>
          </table>
        </div>
