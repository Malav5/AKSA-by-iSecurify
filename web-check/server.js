...
const port = process.env.PORT || 3001; // The port to run the server on
const API_DIR = '/api'; // Name of the dir containing the lambda functions
const dirPath = path.join(__dirname, 'api'); // Path to the lambda functions dir
...
if (process.env.DISABLE_GUI && process.env.DISABLE_GUI !== 'false') {
  ...
  app.use(async (req, res, next) => {
    ...
    import(pathToFileURL(ssrHandlerPath).href).then(({ handler: ssrHandler }) => {
