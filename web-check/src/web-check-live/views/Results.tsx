...
const Results = (props: { address?: string } ): JSX.Element => {
  ...
  const urlParam = props.address || useParams().urlToScan || '';
  const [selectedDomain, setSelectedDomain] = useState(urlParam);
  const [domainInput, setDomainInput] = useState(urlParam);
  ...
  // When the URL param changes (e.g. on first load), update state
  useEffect(() => {
    setSelectedDomain(urlParam);
    setDomainInput(urlParam);
  }, [urlParam]);
  ...
  const handleDomainSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    let value = domainInput.trim();
    if (value.endsWith('/')) value = value.slice(0, -1);
    const addressType = determineAddressType(value);
    if (addressType === 'empt') {
      setDomainError('Field must not be empty');
    } else if (addressType === 'err') {
      setDomainError('Must be a valid URL, IPv4 or IPv6 Address');
    } else {
      if (addressType === 'url' && !/^https?:\/\//i.test(value)) {
        value = 'https://' + value;
      }
  ...
  const [ipAddress, setIpAddress] = useMotherHook({
    jobId: 'get-ip',
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/get-ip?url=${selectedDomain}`)
  ...
  useEffect(() => {
    if (!addressType || addressType === 'empt') {
      setAddressType(determineAddressType(selectedDomain || ''));
    }
    if (addressType === 'ipV4' && selectedDomain) {
      setIpAddress(selectedDomain);
    }
  }, [selectedDomain, addressType, setIpAddress]);  
  ...
  const [sslResults, updateSslResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/ssl?url=${selectedDomain}`).then((res) => parseJson(res)),
  });
  ...
  const [domainLookupResults, updateDomainLookupResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/whois?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [lighthouseResults, updateLighthouseResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/quality?url=${selectedDomain}`)
      .then(res => parseJson(res))
  ...
  const [techStackResults, updateTechStackResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/tech-stack?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [cookieResults, updateCookieResults] = useMotherHook<{cookies: Cookie[]}>({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/cookies?url=${selectedDomain}`)
      .then(res => parseJson(res)),
  });
  ...
  const [headersResults, updateHeadersResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/headers?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [dnsResults, updateDnsResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/dns?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [httpSecurityResults, updateHttpSecurityResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/http-security?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [socialTagResults, updateSocialTagResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/social-tags?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [traceRouteResults, updateTraceRouteResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/trace-route?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [dnsServerResults, updateDnsServerResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/dns-server?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [firewallResults, updateFirewallResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/firewall?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [dnsSecResults, updateDnsSecResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/dnssec?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [hstsResults, updateHstsResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/hsts?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [threatResults, updateThreatResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/threats?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [mailConfigResults, updateMailConfigResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/mail-config?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [archivesResults, updateArchivesResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/archives?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [rankResults, updateRankResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/rank?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [screenshotResult, updateScreenshotResult] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/screenshot?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [tlsResults, updateTlsResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/tls?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [redirectResults, updateRedirectResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/redirects?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [linkedPagesResults, updateLinkedPagesResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/linked-pages?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [robotsTxtResults, updateRobotsTxtResults] = useMotherHook<{robots: RowProps[]}>({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/robots-txt?url=${selectedDomain}`)
      .then(res => parseJson(res)),
  });
  ...
  const [serverStatusResults, updateServerStatusResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/status?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [whoIsResults, updateWhoIsResults] = useMotherHook<Whois | { error: string }>({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`https://api.whoapi.com/?domain=${selectedDomain}&r=whois&apikey=${keys.whoApi}`)
      .then(res => parseJson(res))
  ...
  const [txtRecordResults, updateTxtRecordResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/txt-records?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [blockListsResults, updateBlockListsResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/block-lists?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [sitemapResults, updateSitemapResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/sitemap?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [carbonResults, updateCarbonResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/carbon?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  const [siteFeaturesResults, updateSiteFeaturesResults] = useMotherHook({
    ...
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/features?url=${selectedDomain}`)
    .then(res => parseJson(res))
    .then(res => {
  ...
  // Get a websites listed pages, from security.txt
  const [securityTxtResults, updateSecurityTxtResults] = useMotherHook({
    jobId: 'security-txt',
    updateLoadingJobs,
    addressInfo: { address: selectedDomain, addressType, expectedAddressTypes: urlTypeOnly },
    fetchRequest: () => fetch(`${api}/security-txt?url=${selectedDomain}`).then(res => parseJson(res)),
  });
  ...
  return (
    <ResultsOuter>
      <form onSubmit={handleDomainSubmit} style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: '1rem 0' }}>
        <input
          type="text"
          value={domainInput}
          onChange={handleDomainChange}
          placeholder="Enter or change domain (e.g. https://duck.com/)"
          style={{ padding: '0.5rem', fontSize: '1rem', width: '350px' }}
        />
        <button type="submit" style={{ padding: '0.5rem 1rem', fontSize: '1rem' }}>Go</button>
        {domainError && <span style={{ color: 'red', marginLeft: '1rem' }}>{domainError}</span>}
      </form>
      <Nav>
      { selectedDomain && 
        <Heading color={colors.textColor} size="medium">
          { addressType === 'url' && <a target="_blank" rel="noreferrer" href={selectedDomain}><img width="32px" src={`https://icon.horse/icon/${makeSiteName(selectedDomain)}`} alt="" /></a> }
          {makeSiteName(selectedDomain)}
        </Heading>
        }
      </Nav>
      ...
      <AdditionalResources url={selectedDomain} />
      <Footer />
